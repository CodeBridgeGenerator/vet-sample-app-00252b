...
import catFakerFactory from "./catFakerFactory";
...
const CatFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = catFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cat").create(elem));
...
export default connect(mapState, mapDispatch)(CatFakerDialogComponent);
