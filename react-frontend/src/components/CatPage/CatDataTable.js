...
const CatDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const p_numberTemplate2 = (rowData, { rowIndex }) => <p >{rowData.age}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "cat"});
...
<Column field="age" header="Age" body={p_numberTemplate2} filter={selectedFilterFields.includes("age")} hidden={selectedHideFields?.includes("age")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Cat Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="cat"            
          onUploadComplete={() => {
      ...
      <Dialog header="Search Cat" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
