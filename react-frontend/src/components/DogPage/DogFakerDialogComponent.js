...
import dogFakerFactory from "./dogFakerFactory";
...
const DogFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = dogFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("dog").create(elem));
...
export default connect(mapState, mapDispatch)(DogFakerDialogComponent);
