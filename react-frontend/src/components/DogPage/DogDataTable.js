...
const DogDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.breed}</p>
const p_numberTemplate2 = (rowData, { rowIndex }) => <p >{rowData.age}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "dog"});
...
<Column field="breed" header="Breed" body={pTemplate1} filter={selectedFilterFields.includes("breed")} hidden={selectedHideFields?.includes("breed")}  sortable style={{ minWidth: "8rem" }} />
<Column field="age" header="Age" body={p_numberTemplate2} filter={selectedFilterFields.includes("age")} hidden={selectedHideFields?.includes("age")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Dog Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="dog"            
          onUploadComplete={() => {
      ...
      <Dialog header="Search Dog" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
